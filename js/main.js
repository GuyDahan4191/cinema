...
function onHighlight() {
    var elseat = document.querySelector('h2 span')
    var elseatInx = elseat.innerText
    var pos = elseatInx.split('-')
    // console.log('pos:', pos)
    highlightAvailableSeatsAround(gCinema, +pos[0] - 1, +pos[1] - 1)
    // console.log('+pos[0]-1:', +pos[0] - 1)
    // console.log('+pos[1]-1:', +pos[1] - 1)
}
...
function highlightAvailableSeatsAround(board, rowIdx, colIdx) {
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            var currCell = board[i][j]
            if (currCell.isSeat && !currCell.isBooked) {
                const elSeatToLight = document.querySelector(`.cell[data-i="${i}"][data-j="${j}"]`)
                elSeatToLight.classList.add('highlight')
                setTimeout(() => {
                    elSeatToLight.classList.remove('highlight')
                }, 2500)
            }
        }
    }
